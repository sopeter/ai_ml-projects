...
def breadthFirstSearch(problem: SearchProblem):
    ...
    while True:
        ...
        if successors:
            for next_state, action, step_cost in successors:
                if next_state not in explored and next_state not in (state[0] for state in frontier.list):
                    frontier.push((next_state, path + [action]))
...
def uniformCostSearch(problem: SearchProblem):
    ...
    while True:
        ...
        curr_node, path, step_cost = frontier.pop()
        explored.append(curr_node)
        ...
        if successors:
            for next_state, action, step_cost in successors:
                next_cost = problem.getCostOfActions(path) + step_cost
                next_node = (next_state, path + [action], next_cost)

                if next_state not in explored:
                    if next_state not in (state[2][0] for state in frontier.heap):
                        frontier.push(next_node, next_cost)
                    elif next_state in (state[2][0] for state in frontier.heap):
...
def aStarSearch(problem: SearchProblem, heuristic=nullHeuristic):
    ...
    frontier.push(init_node, heuristic(problem.getStartState(), problem))
